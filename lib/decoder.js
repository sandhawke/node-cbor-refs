...
class Decoder extends BinaryParseStream {
   ...
   * @param {array} options.kept - array of objects used for value-sharing
   *   with semantic tags 28+29.  Use this option, passing in an empty array,
   *   if you want to see what values were marked for sharing.  You can then
   *   pass it to the next decoder, to retain value-sharing state.
   */
  constructor (options) {
    options = options || {}
  ...
  static decodeAllSync (input, options) {
    ...
    if (c.hasUnresolvedReferences) {
      c.hasUnresolvedReferences = !!resolve.resolveReferences(res, c.kept)
    }
    return res
  }
      ...
      while (parent != null) {
        ...
        if (Array.isArray(parent)) {
          ...
          case MT.TAG:
            ...
            const opt = { kept: this.kept} 
            val = t.convert(this.tags, opt)
            // we're using opt as a way to indirectly return the fact
            // that we've hit an unresolvable reference.  Should we
            // instead just pass 'this' to t.convert?   That would be
            // simpler, but more prone to abuse.
            if (opt.hasForwardReferences) {
              this.hasUnresolvedReferences = true
            }
            break
          }
