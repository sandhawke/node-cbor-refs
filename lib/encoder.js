...
class Encoder extends stream.Transform {
  ...
  _pushObject (obj) {
    ...
    //console.log('pushObject', obj, this._kept, this._pleaseKeep)
    let i
    if (-1 !== (i = this._kept.indexOf(obj))) {
      //console.log('ref back to', i)
      this._pushTag(29)
      this._pushInt(i)
      return true
    }
    if (-1 !== (i = this._pleaseKeep.indexOf(obj))) {
      ...
      //console.log('establishing refback #', i)
      this._pleaseKeep.splice(i, 1)
      this._pushTag(28)
      // this._pushInt(at) this number is implied; DONT send it
      // keep going, and actually serialize the value
    }

    if (this._pushing) {
      if (this._pushing.indexOf(obj) > -1) {
        //console.log('found loop at', obj)
        this._onCycle(obj)
        // not reached; onCycle should throw something
        return
      }
      
      this._pushing.push(obj)
      const value = this._pushObjectStage2(obj)
      this._pushing.pop()
      return value
    } else {
      return this._pushObjectStage2(obj)
    }
  }

  _pushObjectStage2 (obj) {
   ...
   */
  static encodeAllWithCycles (objs, options) {
    const opt = {}
    // remove the cycles:true that landed us here, or we'll
    // be looping back again when we call encodeAll
    Object.assign(opt, options || {}, {cycles: undefined})
    opt.pleaseKeep = opt.pleaseKeep || []
    opt.onCycle = x => { throw { cycleFound: x } }
    while (true) {
      // Try serializing, but catch the special object throw on
      // discovering a cycle.  Add the problematic object to
      // pleaseKeep, so the next time through it should be marked as
      // shareable, and thus not cause an error.
      //
      // This might be more complicated than scanning for cycles
      // before we serialize, but it means one traversal in the normal
      // non-cycle case. Plus, this means only one copy of the
      // traveral code.
      try {
        //console.log('encodeAll...')
        return Encoder.encodeAll(objs, opt)
      } catch (e) {
        if (!e.cycleFound) throw e
        opt.pleaseKeep.push(e.cycleFound)
        //console.log('found cycle, trying again')
        //console.log('keeps now:', opt.pleaseKeep)
      }
    }
  }
  ...
  static encodeAll (objs, options) {
    if (options && options.cycles) {
      return Encoder.encodeAllWithCycles(objs, options)
    }
    //console.log('encodeAll...')
    ...
    enc.end()
    //console.log('encodeAll returning', bs)
    return bs.read()
  }
   ...
   */
  static encode () {
    return Encoder.encodeAll(Array.prototype.slice.apply(arguments))
  }
}
